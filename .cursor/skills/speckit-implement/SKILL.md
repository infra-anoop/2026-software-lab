...
- **Open Decisions (§G / §G.1 / §G.2):** Before first product implement for a feature, require `FINISH_BAR.md` with **Implement unblocked: yes** (§G.2). Before a task that depends on deferred content, read Open Decisions — if `who: human` and `status: open`, **STOP**. After architecture-affecting locks, complete **§G.1** (`PLAN_DELTA.md` …) before dependent packets. `[HITL]` / `[OD:D#]` hard stops. `waived` = out of version.
...
6. Execute implementation following the task plan:
   ...
   - **Open Decisions:** Do not execute product work that invents content for open `who: human` rows (§G). Refuse feature implement until `FINISH_BAR.md` shows **Implement unblocked: yes** (§G.2). After architecture-affecting locks, refuse dependent impl until `PLAN_DELTA.md` §G.1 checklist is complete.
