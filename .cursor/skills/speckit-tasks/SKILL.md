...
4. **Generate tasks.md**: Use TASKS_TEMPLATE_CONTENT (from the JSON output above) as the structure. For compatibility with older setup scripts that omit TASKS_TEMPLATE_CONTENT, read TASKS_TEMPLATE instead. Fill with:
   ...
   - Each phase includes: story goal, independent test criteria, tests (required for executable apps), implementation tasks
...
**Tests are REQUIRED for executable apps** (registry HTTP/jobs/UI): generate contract-test and catalog-auto test tasks from `contracts/` hooks and `acceptance.md` `how: auto` rows. Place them **before** matching implementation tasks. Do **not** omit tests unless the feature is docs-only / non-executable (state that exception in Completion Report).
...
1. **From User Stories (spec.md)** - PRIMARY ORGANIZATION:
   ...
   - Map all related components to their story:
     ...
     - Tests for that story (REQUIRED for executable apps: contract + catalog-auto)
   - Mark story dependencies (most stories should be independent)

2. **From Contracts**:
   - Map each interface contract → to the user story it serves
   - Each interface contract → contract test task [P] before implementation in that story's phase (executable apps)
...
- **Phase 3+**: User Stories in priority order (P1, P2, P3...)
  - Within each story: Tests (required for executable apps) → T* review when contract/auto tests exist → Models → Services → Endpoints → Integration
