...
**Meaning (2026-09-20):** “Deferred earlier” meant *not right then*, **not** “drop from V2 finish.” Remaining `open` rows stay **required** for Smart Writer V2 complete dogfood unless set to `waived` (out of version). Sequencing tasks ≠ skipping scope. After architecture-affecting locks → §G.1 [`PLAN_DELTA.md`](./PLAN_DELTA.md).

| id | shape_locked | content_open | who | before | status | arch_impact |
|----|--------------|--------------|-----|--------|--------|-------------|
| **D1** | Closed property vocabulary; dual intake axes (F6); `factual` = tone only (F3) | Exact label list for v2.0 seed | human | Before changing `app/properties.py` / steering chips beyond the ratified seed | **locked** — ratified seed: `factual`, `persuasive`, `concise`, `warm`, `formal`, `humorous`, `specific`, `urgent` | content-only |
| **D2** | Preview gate + rate limit + queue exist (B5/A9) | Numeric turn/cost caps | human | Before T063 | **locked** (2026-09-20) — max **3** write jobs / conversation; max **10** clarify turns / conversation; max **8** inner writer↔assessor turns / write job. Fail closed when hit. | content-only |
| **D3** | `LOGFIRE_TOKEN` named in secrets schema (A25); no product SC for Logfire (F5) | Wire observability for V2 finish + which debug fields | human | Before T064 | **locked** (2026-09-20) — **Wire for V2 finish** (required for dogfood debug). Init when token present; **noop if unset** (product still runs). Minimum job/run visibility: **elapsed time** and **tokens and/or estimated cost** (best-effort from provider usage). Traces must not log secrets or raw InternalRunState. Flag-based log levels / fancy filters = later, not this lock. | architecture-affecting |
| **D4** | Citation modes = panel / inline / footnotes / combo; default panel when sources exist; skip ask if no sources (F7) | Where the override lives in UI | human | Before T065 | **locked** (2026-09-20) — **Settings panel** (not in-chat). Citation format is one settings preference; same panel is the home for similar prefs that will accrue. Default remains **sources panel**. No mandatory per-turn citation interview. Refine layout during Vercel/v0-class GUI design (**D5**). | content-only |
| **D5** | Chat UI is Next.js (v0-designed); Railway hosts the shipped product UI; preview gate exists | Topology + who holds the audit secret | human | Before T069 / GUI design | **re-locked** (2026-09-21) — **Design** with **Vercel v0** (keep). **Deploy:** **one** Railway service `smart-writer-v2` serves **UI + API same-origin** (FastAPI serves built Next assets; no second `smart-writer-v2-ui` service required). **Secret custody = V1-style:** user enters shared preview secret in the UI; browser sends `X-Audit-Secret`; worker env is source of truth. **Not** BFF-held silent secret (public UI + server secret does not gate spend). **Not** Vercel as production host; **not** Codespaces-only as the product. Overturns 2026-09-20 “BFF / browser never sees secret / two Railway surfaces” lock. | architecture-affecting |
| **D6** | Preview secret **name** in schema; mutating `/v1` fail-closed without it | Seed `SMART_WRITER_V2_AUDIT_SECRET` in Infisical (single Railway worker/UI service) | human | Before T070 / mutating prod | **locked** (2026-09-20; **custody note amended 2026-09-21** with **D5**) — **Yes, seed for V2 finish.** Human puts value in Infisical; sync to the **one** Railway service. Users type the same shared secret in the UI (V1-style). Mutating `/v1` must work in prod (not only `/health`). Never commit the value. | content-only |
| **D7** | Materials via **links** in MVP; uploads first-class in F4 | File-upload HTTP in V2 finish scope (yes + shape) vs out-of-version waive | human | Before T073+ | **locked** (2026-09-20) — **In V2 finish.** Upload HTTP + chat UI control. **Storage = in-memory bytes** on the worker (restart loses files, same residual as conversations). Sensible size/MIME limits at impl (PDF/text-class); no object store required for this version. | architecture-affecting |
| **D8** | F6 dual axes; F8 human revise distinct from machine critique; FR-006 weave into draft/critique | Dual-axis scored inner loop required for complete V2? | human | Before T078+ | **locked** (2026-09-20) — **Required** for V2 complete. Each write job: rubric from **Axis A (intent) + Axis B (properties)**; assessor **scores**; optimize ≤8 turns. Redesign OK; **not thinner** than V1 scored loop. Clarifiers exist to nail those dimensions for the rubric. Overturns plan/research “out of MVP / optional P3.” | architecture-affecting |
| **D9** | Outer revise path exists (FR-019); continuity vs regenerate locked (F8); UI regenerate control exists (T038) | Tactical vs fuller rerun on normal feedback; how user indicates start over | human | Before T087+ | **locked** (2026-09-20) — **Simple default:** all normal feedback (`client_intent=auto`) uses the **same revise** path (continuity / “tactical”; no LLM routing between tactical vs fuller rerun). **Start over** = **explicit UI control** (and API `client_intent=regenerate`) → fresh **generate**, new chain head. **Do not** rely on the LLM to infer “start over” from free-form prose for this version (that would be heuristic routing ≈ option 2). Free-form feedback always revises unless the user hits regenerate. | content-only |

**Fail closed:** Do not invent `content_open` while `open`. Pose in product language; lock here; §G.1 if architecture-affecting; then implement.
...
7. Preview gate / jobs → implemented; vault seed → **D6** (locked); UI = **v0 + one Railway service + V1-style secret** → **D5** (re-locked).
