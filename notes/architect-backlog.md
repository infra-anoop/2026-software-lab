...
| **A23** | **Conscious trigger only** for secrets install/rotate: **`workflow_dispatch` OR ops tag push** (`sync/<app_id>/<environment>` via `ops-runtime.yml`). **Not** post-`v*` ship. **Not** auto on ordinary branch push / PR. Code CI (verify/ship/deploy) stays image/health-only. |
...
| **Lifecycle** | **Bootstrap** (A26 provision → A23 sync → A24 verify via `bootstrap/<app>/<env>` tag or UI) ≠ **rotate** (`sync/<app>/<env>` tag or UI) ≠ **code deploy** (`v*` image pin). Conscious choice to init deploy infra. |
...
| A26 | Platform provision CLI (`scripts/provision_runtime.py`) + `provision-runtime.yml` (`workflow_dispatch` break-glass) + preferred **`bootstrap/<app>/<env>`** ops tag (`ops-runtime.yml`: provision→sync→verify). Idempotent Railway ensure project→env→service from deploy YAML; dry-run/apply mutually exclusive; mocked create+exists tests; no live Railway in CI. Empty service until deploy pins image. |
...
| A23 | Secrets install/rotate: preferred **`sync/<app>/<env>`** ops tag (`ops-runtime.yml` + `scripts/ops_runtime_tag.py`) **or** `sync-runtime-secrets.yml` `workflow_dispatch` break-glass. **Not** on `v*` / branch push / PR. Apply → Railway token + Infisical OIDC (`infisical_oidc_login.py` → `INFISICAL_TOKEN`) or non-preferred `secrets.INFISICAL_TOKEN`; exit 2 on missing footprint. |
| A24 | `verify_runtime_bootstrap.py` + `verify-runtime-bootstrap.yml` (`workflow_dispatch` break-glass) + live verify at end of **`bootstrap/**`** ops tag: read-only footprint + required names (YAML ∪ schema); `variables` query names-only; exit 2 footprint miss; mocked tests. |
